...
class TrainingGoals {
    ...
    deleteEntriesByUser(_id, user) {
        this.db.remove({'_id': _id}, {}, function(err, docsRem) {
            if (err) {
                console.log('Error deleting user entry.');
            } else {
                console.log(docsRem, 'Entry removed from the database for user: ' + user);
            }
        })
    }

    updateEntriesByUser(_id, user) {
        this.db.update({'_id': _id}, {}, function(err, docsRem) {
            if (err) {
